# CMMF Translation Contract 1.2.0
...
For CMMF 1.2.0, translators should treat field-level `list` as the canonical repeatability signal. Translators may also accept legacy `relation.cardinality` on reference fields for compatibility.
...
- whether legacy `relation.cardinality` input was normalised to `list`
...
- silently accept conflicting `list` and legacy `relation.cardinality` values on the same field
